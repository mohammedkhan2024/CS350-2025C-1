...
class TemperatureMachine(StateMachine):
    ...
    def on_enter_heat(self):
        ...
        self.updateLights() # update indicator light for heat state
    ...
    def on_exit_heat(self):
        ...
        redLight.off() # turn off red LED when exiting heat state
    ...
    def on_enter_cool(self):
        ...
        self.updateLights()  # update indicator light for cool state
        if(DEBUG):
    ...
    def on_exit_cool(self):
        ...
        blueLight.off()  # turn off blue LED when exiting cool state
    ...
    def on_enter_off(self):
        ...
        redLight.off() # Turn off the red light

        blueLight.off() # Turn off the blue light
    ...
    def processTempStateButton(self):
        ...
        self.cycle()  # cycle to the next state
    ...
    def processTempDecButton(self):
        ...
        self.updateLights() # Update lights
    ...
    def updateLights(self):
        ...
        temp = floor(self.getFahrenheit()) # get current temp
        redLight.off() # Turn off red light
        blueLight.off() # Turn off blue light
        ...
        if self.current_state.id == "off": # If state is off

            redLight.off() # Turn off red light

            blueLight.off() # Turn off blue light

        elif self.current_state.id == "heat": # If state is heat

            if temp < self.setPoint: # If current temp less than set point

                redLight.pulse()  # fading red LED if temperature below set point

            else: # if current temp greater or equal to set point

                redLight.on() # solid red LED if temperature meets/exceeds set point

            blueLight.off() # Turn off blue light

        elif self.current_state.id == "cool": # If state is cool

            if temp > self.setPoint: # If current temp greater than set point

                blueLight.pulse()  # fading blue LED if temperature above set point

            else: # if current temp less than or equal to setpoint

                blueLight.on()     # solid blue LED if temperature meets/exceeds set point
    ...
    def getFahrenheit(self):
        t = thSensor.temperature # Read temperature from sensor
    ...
    def setupSerialOutput(self):
        ...
        output = f"{self.current_state.id},{floor(self.getFahrenheit())},{self.setPoint}" # Create output string
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            lcd_line_1 = current_time.strftime("%Y-%m-%d %H:%M:%S")  # line 1: date and time

            ## Setup Display Line 2
            if(altCounter < 6): # Display temp for 5 seconds

                lcd_line_2 = f"Temp: {floor(self.getFahrenheit())}F"  # line 2: current temperature

                altCounter = altCounter + 1 # Increment counter
            else: # Display state/setpoint for 5 seconds

                lcd_line_2 = f"S {self.current_state.id} SP {self.setPoint}F"  # line 2: state and setPoint

                altCounter = altCounter + 1 # increment counter
                if(altCounter >= 11): # Reset counter and update lights
                    # Run the routine to update the lights every 10 seconds
                    # to keep operations smooth
                    self.updateLights() # Update lights
                    altCounter = 1 # Reset counter
            ...
            if((counter % 30) == 0): # Check if 30 seconds have passed

                ser.write(self.setupSerialOutput().encode())  # send state update over serial
...
greenButton.when_pressed = tsm.processTempStateButton  # cycle thermostat state
